...
void render(const renderstate_t& state, int w, int h, addr_t vgatext_base){
	...
	// {
		else if(state.mode == 0x1 || state.mode == 0x2)
		{
    ...
    else if(state.mode == 0x3)
    {
      if(state.input_key != 10)
      {
        if(state.input_key != 8) //Backspace key (ASCII)
          vgatext::writechar((y)*w+x, state.input_key, (uint8_t)0x0, (uint8_t)0x7, vgatext_base);
        else
          vgatext::writechar((y)*w+x+1, (int)'\0', (uint8_t)0x0, (uint8_t)0x7, vgatext_base);
      }
    }
		else
		{
