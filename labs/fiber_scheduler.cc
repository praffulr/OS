...
void pfactorsf3(shellstate_t* pshellstate, int* pn, addr_t* pmain_stack, addr_t* pf_stack)
{
	...
	*/
	...
	shellstate.fiber_isFirst[shellstate.head] = false;
	for(int i=2; i<= n; i++)
	{
		bool is_prime = true;
		// check if i is prime
		...
		if(is_prime && (n%i == 0))
		{
			...
			int_to_string_write3(shellstate.fiber_output[shellstate.head] + output_iterator, i);
			output_iterator += 1 + num_digits;
			shellstate.fiber_output[shellstate.head][output_iterator-1] = ' ';
		}
		// return to main
		stack_saverestore(f_stack, main_stack);
	}
	shellstate.fiber_output[shellstate.head][output_iterator] = '\0';
	shellstate.fiber_isFirst[shellstate.head] = true;
				...
				else
				{
					...
					else
					{
						...
						if(shellstate.fiber_isDone[shellstate.head])
						{
							...
							hoh_debug(shellstate.head);
							hoh_debug(shellstate.fiber_output[shellstate.head]);
